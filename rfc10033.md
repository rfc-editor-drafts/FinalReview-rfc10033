...
informative:
  ...
  MCGREW:
    ...
    author:
      ...
      - ins: J. Buchmann
    date: 2016-11-02
    refcontent: Cryptology ePrint Archive, Paper 2016/357, Security Standardization Research 2016
  ...
  BH16:
    ...
    author:
      ...
      -
        ins: A. Hülsing
    date: 2016
    refcontent: Cryptology ePrint Archive, Paper 2016/1042, Selected Areas in Cryptography 2017

  Fluhrer23:
    target: https://eprint.iacr.org/2023/1905
    ...
    refcontent: Selected Areas in Cryptography - SAC 2017, August 16-18, pp. 299–322 

  ETSI-TR-103-692:
    target: https://www.etsi.org/deliver/etsi_tr/103600_103699/103692/01.01.01_60/tr_103692v010101p.pdf
    title: CYBER; State management for stateful authentication mechanisms
    author:
      -
        org: ETSI
    date: 2021-11
    refcontent: ETSI TR 103 692, V1.1.1
  ...
  BSW16:
    ...
    author:
      ...
      - ins: B. Waters
    date: 2016-04-28
    refcontent: "Advances in Cryptology – EUROCRYPT 2016, Lecture Notes in Computer Science, vol. 9666, pp. 792-821"
    seriesinfo:
      DOI: 10.1007/978-3-662-49896-5_28
...
in {{alt-backup-mgmt}} that are not covered in prior work.
...
assumptions, such as ML-DSA {{FIPS204}}. However, if run time, implementation
...
In particular, the state and private key might be inseparable in implementations
...
early and corrected before a catastrophic loss of security can occur (e.g., accidentally instantiating multiple instances of a Stateful HBS key/state).
...
thereby undermining the intended separation.
...
Given the flexibility of these schemes -- such as adjustable tree heights or Winternitz parameters -- there
...
or live cloning of a VM can easily lead to a state reuse {{MCGREW}}. With users
...
the signer to never reuse the nonce.
...
{{SP.800-208}} proposes generating multiple Stateful HBS key pairs and
...
The proposal of using multiple Stateful HBS key pairs for a single instance also
...
Each of the N sectors' OTS private key values can be generated pseudorandomly
...
allows, is described in {{alt-backup-mgmt}}, where key transfer is
...
In most use cases, key rotation is not foolproof since it will require
...
For example, this scenario may happen if all hardware modules are broken at
the same time.
...
Now this bottom-level tree can be used to sign messages. As soon as no more OTSs
